...
def AsitIsZmat(file, opt, res):
    iform = file.split('.')

    # CREATE A MOL FILE FROM ANY FILE
    if file.endswith('smi'):
        os.system('babel -i%s %s -omol %s.mol --gen3D' % (iform[1], file, iform[0]))
    else:
        os.system('babel -i%s %s -omol %s.mol ---errorlevel 1 -b &>LL' % (iform[1], file, iform[0]))
    ...
    print_ZMAT(ATYPES, G_mol, mol_icords, COOS, '%s.z' % res, res)
...
def ReadMolFile(mollines):
    ...
    for line in bondlines:
        ...
        bonds['BI'].append(bi)
        bonds['BJ'].append(bj)
        bonds['RIJ'].append(al.Distance(coos[bi], coos[bj]))
...
def GenMolRep(file, opt, res, charge):
    iform = file.split('.')
    try:
        AsitIsZmat(file, opt, res)
    ...
    if os.path.exists('/tmp/clu.pdb'):
        os.system('rm /tmp/clu.pdb')

    if iform[1] == 'pdb':
        if os.environ.get('MCPROdir') is not None:
            os.system('$MCPROdir/miscexec/clu -t:f=pdb %s.pdb -r %s.z -n:f=p /tmp/clu.pdb -m ma' % (iform[0], res))
        else:
            execfile = os.environ['BOSSdir'] + '/scripts/xSPM > /tmp/olog'
            commad = execfile + ' ' + res
            os.system(commad)
...
def make_graphs(atoms, coos, bonds):
    ...
    for i in coos.keys():
        G.add_node(
            ...
            XYZ=coos[i],
            elem=atoms[i],
            atno=al.bossElement2Num(atoms[i])
        )
...
def print_ZMAT(atoms, G_mol, mol_icords, coos, zmat_name, res):
    ...
    for i in G_mol.nodes():
        if n_ats > 2:
            ...
            dihed = al.dihedral(coos[ti], coos[tj], coos[tk], coos[tl])
